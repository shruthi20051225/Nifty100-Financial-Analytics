...
def create_sector_report(sector_name, sector_df):
    ...
    pdf_path = os.path.join(OUTPUT, f"{sector_name}_report.pdf")
    ...
    story.append(
        Paragraph(
            f"<b><font size=18>{sector_name} Sector Report</font></b>", styles["Title"]
        )
    )
    ...
    story.append(Paragraph("<b>Sector Summary</b>", styles["Heading2"]))

    story.append(Paragraph(f"Total Companies : {len(sector_df)}", styles["BodyText"]))
    ...
    table_data = [["Company", "ROE", "ROCE", "PE", "PB"]]

    for _, company in sector_df.iterrows():

        cid = company["id"]

        ratio = ratios[ratios["company_id"] == cid].sort_values("year")

        market_data = market[market["company_id"] == cid].sort_values("year")
        ...
        if market_data.empty:
            ...
            pe = latest_market.get("pe_ratio", "")

            pb = latest_market.get("pb_ratio", "")

        table_data.append([company["company_name"], roe, roce, pe, pb])
        # ---------------------------------------
    # TABLE STYLE
    # ---------------------------------------
    ...
    table = Table(
        table_data,
        colWidths=[3.2 * inch, 0.9 * inch, 0.9 * inch, 0.9 * inch, 0.9 * inch],
    )
...
if sector_column not in companies.columns:
    ...
    for sector in sorted(companies[sector_column].dropna().unique()):
        ...
        sector_df = companies[companies[sector_column] == sector]

        create_sector_report(sector, sector_df)
