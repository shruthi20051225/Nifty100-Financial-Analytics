...
ratios = ratios[ratios["year"] == "2024-03-01 00:00:00"]

market = market[market["year"] == 2024]
...
df["FCF Yield (%)"] = (df["free_cash_flow_cr"] / df["market_cap_crore"]) * 100
...
df["Valuation Flag"] = df["pe_ratio"].apply(valuation)

# ---------------- PE vs Median ----------------

median_pe = df["pe_ratio"].median()

df["PE vs Median (%)"] = ((df["pe_ratio"] - median_pe) / median_pe) * 100
...
summary[summary["Valuation Flag"] != "Fair"].to_csv(
